...
//iceServerConfig.iceServers.unshift({url: 'turn:game-of-business.com:3478'});
stun.forEach(function (url) {
    iceServerConfig.iceServers.push({url: 'stun:' + url});
});


function Peer(source_id, servers) {
    this.source_id = source_id || localStorage.user_id;
    var options = {
        optional: [{RtpDataChannels: true}]
    };
    this.connection = new RTCPeerConnection(servers, options);
    this._candidates = [];
}
...
Peer.prototype = {
    ...
    offer: function () {
        ...
        this.connection.createOffer(function (offer) {
            self.connection.setLocalDescription(offer, function () {
                server.send(self.target_id, offer)
            }, morozov);
        }, morozov, {
    ...
    dataChannel: function (name) {
        ...
        channel.addEventListener('open', morozov);
    ...
    server.on('offer', function (offer) {
        ...
        peer.connection.createAnswer(function (answer) {
            peer.connection.setLocalDescription(answer, function () {
                    server.send(peer.target_id, answer)
                },
                morozov);
        });
    });
